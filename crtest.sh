...
if command -v pm2 > /dev/null 2>&1 && [[ $(which pm2) == "${USER_HOME}/.npm-global/bin/pm2" ]]; then
  ...
  if [ -f "${WORKDIR}/start.sh" ] && [ -f "${FILE_PATH}/config.json" ] && [ -f "$HYSTERIA_CONFIG" ]; then
    ...
    (crontab -l 2>/dev/null | grep -F "*/1 * * * * pgrep -f \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") || \
      (crontab -l 2>/dev/null; echo "*/1 * * * * pgrep -f \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") | crontab -
    (crontab -l 2>/dev/null | grep -F "*/1 * * * * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") || \
      (crontab -l 2>/dev/null; echo "*/1 * * * * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") | crontab -
    (crontab -l 2>/dev/null | grep -F "*/1 * * * * pgrep -x \"web\" > /dev/null || ${CRON_HYSTERIA}") || \
      (crontab -l 2>/dev/null; echo "*/1 * * * * pgrep -x \"web\" > /dev/null || ${CRON_HYSTERIA}") | crontab -
